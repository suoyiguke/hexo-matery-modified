---
title: DATETIME-vs-TIMESTAMP-vs-INT，怎么选？.md
top: false
cover: false
toc: true
mathjax: true
...
###业务表结构设计实战，DATETIME vs TIMESTAMP vs INT，怎么选？
...
2、5.7 DATETIME用int存储了，占5个字节、DATETIME(6) 占用 8 个字节，TIMESTAMP 占用 4 个字节，DATETIME(6) 依然占用 8 个字节，TIMESTAMP(6) 占用 7 个字节；
...
###思考题
若设置字段为 DATETIME(6)，时间精确到小数点后 6 位，在这么高精度的存储下，请问存储的值是否是唯一的？我们又如何验证呢？认真思考并在留言区回答哦，我会进行点评，下一讲见。

不是惟一的。
~~~
mysqlcreate table t(id bigint auto_increment primary key,dt datetime(6));

# mysqlslap -S /data/mysql/mysql3306/mysql3306.sock --number-of-queries=100000 --concurrency=100 --query='insert into t(dt) select NOW(6)';
~~~
mysql不唯一，100个并发插入10万条记录，字段 dt 值重复出现大于1次的有1000条，也就是 1000/100000，大概 1% 的重复率
...
###实践
给dd表添加一个update_date列。默认值为6位数毫秒的值，且每次修改自动更新
~~~
ALTER TABLE `test`.`dd` 
ADD COLUMN `update_date` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6);
~~~
